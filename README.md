...
### 3. Area별 Windows DNS 서버 구축 및 VCSA 레코드 등록

WS-ESXi 위에서 구동될 각 Area의 VM (Windows Server 2022)들을 독립된 DNS 서버로 설정합니다.
...
* VCSA (vCenter Server Appliance) 배포를 위한 사전 레코드 설정
...
* Internet Access (NAT): Seoul 라우터의 `eth2`를 외부 Internet과 연결하고, Source NAT 설정을 통해 두 Area의 모든 Internal Network VM이 Internet을 사용할 수 있도록 합니다.
...
* Hybrid LAN 인터페이스 설정
    ...
    * 동시에 `vif 20`, `vif 30` 설정을 통해 802.1Q 태깅이 적용된 서브 인터페이스를 생성합니다.
...
| Area | ESXi Host | IP | Gateway | DNS Server |
| --- | --- | --- | --- | --- |
| Seoul | Seoul-ESXi-01 | `10.30.10.10` | `10.30.10.1` | `10.30.10.77` |
| Seoul | Seoul-ESXi-02 | `10.30.10.20` | `10.30.10.1` | `10.30.10.77` |
| Seoul | Seoul-ESXi-03 | `10.30.10.30` | `10.30.10.1` | `10.30.10.77` |
| Jeju | Jeju-ESXi-01 | `172.30.10.10` | `172.30.10.1` | `172.30.10.77` |
| Jeju | Jeju-ESXi-02 | `172.30.10.20` | `172.30.10.1` | `172.30.10.77` |
| Jeju | Jeju-ESXi-03 | `172.30.10.30` | `172.30.10.1` | `172.30.10.77` |

#### 5.2 Laptop 네트워크 설정

| Area | Laptop | IP | Gateway | DNS Server |
| --- | --- | --- | --- | --- |
...
* Internal Service Networks (Tagged): 각 Area 내부 트래픽 (Storage, vMotion 등)은 VLAN 20, 30, 40으로 격리하여 보안과 효율성을 높입니다.
...
| Area | Purpose | VLAN ID | Port Group |
| --- | --- | --- | --- |
| Seoul | Management | 0 (Untagged) | `Seoul-VM` |
| Seoul | Storage | 20 | `Seoul-Storage` |
| Seoul | vMotion | 30 | `Seoul-vMotion` |
| Seoul | Fault Tolerance | 40 | `Seoul-FT` |
| Jeju | Management | 0 (Untagged) | `Jeju-VM` |
| Jeju | Storage | 20 | `Jeju-Storage` |
| Jeju | vMotion | 30 | `Jeju-vMotion` |
| Jeju | Fault Tolerance | 40 | `Jeju-FT` |
...
* 모든 내부 Port Group의 VLAN ID는 0으로 설정합니다.
